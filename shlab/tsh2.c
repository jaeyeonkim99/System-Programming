...
int builtin_cmd(char **argv) 
{
    ...
    if(strcmp(argv[0], "quit")==0){
         //terminate & reap all child processes
        for(int i=0; i<MAXJOBS; i++){
                if(jobs[i].pid!=0){
                int pgid;
                if((pgid=getpgid(jobs[i].pid))<0) unix_error("Fail to get process group ID of job");

                //send sigint to job in list and descendents
                if(kill((-1)*pgid, SIGINT)<0) unix_error("quit: Fail to send SIGINT signal");

                int status;
                if(waitpid(jobs[i].pid, &status, WUNTRACED|WCONTINUED)<0) unix_error("quit: waitpid error");
...
void waitfg(pid_t pid)
{
    ...
    if(waitpid(pid, &status, WUNTRACED)<0) unix_error("waitfg: waitpid error");
        ...
        if(job!=NULL&&job->state==FG) deletejob(jobs, pid);
    } 
...
void sigchld_handler(int sig) 
{
    ...
    //-1: any child process / WNOHANG: does not wait for currently running children to terminate)
    while((pid = waitpid(-1, &status, WNOHANG))>0){  //when there exist child process that should be reaped
        //list modification as the job is reaped
        deletejob(jobs, pid);
    }
    
    if(pid<0) unix_error("sigchld_handler: waitpid error");
